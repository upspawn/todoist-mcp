...
export class TodoistToolHandlers {
  ...
  private async getProjectCollaborators(
    args: Record<string, unknown>
  ): Promise<McpToolResponse> {
    const collaborators = await this.apiClient.getProjectCollaborators(
  ...
  private async closeTask(
    ...
    return {
      content: [
        { type: 'text', text: 'Task marked as completed successfully' },
      ],
    };
  }
  ...
  private async listSections(
    ...
    const sections = await this.apiClient.getSections(
  ...
  private async getCompletedTasksByProject(
    args: Record<string, unknown>
  ): Promise<McpToolResponse> {
